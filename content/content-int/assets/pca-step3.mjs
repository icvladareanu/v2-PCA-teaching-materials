...
export default {
  render({ el }) {
    ...
    controls.append(inputRow, btnGen, btnCenter, btnEigen, btnPCs);
    ...
    function generate() {
      ...
      // Clean up arrows & kill animations
    ...
    // =========================
    function pcs() {
      // Clear any existing labels if button clicked multiple times
      state.labels.forEach(label => sceneB.remove(label));
      state.labels = [];

      state.eigenArrows.forEach((arrowGroup, i) => {
        // Reset scale and kill old animations just in case
        ...
        if (i === 0 || i === 1) {
          ...
          // It's PC3: Dim it out by altering its children meshes
      ...
      // To see the GSAP animation play out smoothly, we must continuously render.
      // We add renderAll to the GSAP ticker loop.
